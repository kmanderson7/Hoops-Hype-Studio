...
export const useStudioState = create((set) => ({
    ...
    togglePreset: (presetId) => set((state) => ({
        exportPresets: state.exportPresets.map((preset) => preset.id === presetId
            ? { ...preset, enabled: !preset.enabled }
            : preset),
    })),
    markPresetProgress: (presetId, progress) => set((state) => ({
        exportPresets: state.exportPresets.map((preset) => preset.id === presetId ? { ...preset, progress } : preset),
    })),
