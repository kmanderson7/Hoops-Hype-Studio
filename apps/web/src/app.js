...
export default function App() {
    ...
    useEffect(() => {
        if (!selectedTrackId || !selectedTrack?.previewUrl)
            return;
        // Skip the silent demo WAV — librosa on silence is meaningless.
        if (selectedTrack.previewUrl.startsWith('data:'))
            return;
        let cancelled = false;
        (async () => {
            try {
    ...
    const handleStartRender = () => {
        ...
        (async () => {
            try {
                ...
                const inputSegments = filteredHighlights.map((h) => ({
                    id: h.id,
                    ...
                    bbox: targetJersey && h.featuredBbox && h.featuredBbox.length >= 2
                        ? h.featuredBbox
                        : undefined,
                }));
